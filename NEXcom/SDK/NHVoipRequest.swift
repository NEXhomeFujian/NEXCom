...
//  NEXcom
//
//  Created by csh on 2022/10/21.
//

import UIKit
import Alamofire
class NHVoipRequest: NSObject {

    public static let it: NHVoipRequest = .init()
    private let manager: Alamofire.SessionManager = Alamofire.SessionManager.default
    
    var myBlock: (() -> Void)?
    
    func unBindDevice(SIPNUM: String,SIGN: String,device_id: String,TIMESTAMP: String, resultBlock: (()->Void)?) -> Void {
        ...
        // todo
        var urlSip = ""
        let urlRequest = try? URLRequest(url: urlSip, method: .delete, headers: headers)
        let request = self.manager.request(urlRequest!)
        request.responseJSON { (response) in
//            print("ppppp" + response.response?.statusCode)
    ...
    func bindDevice(SIPNUM: String,SIGN: String,device_id: String,TIMESTAMP: String){
        ...
        // todo
        var urlSip = ""
        var urlRequest = try? URLRequest(url: urlSip, method: .post, headers: headers)
        var lan = "en"
        if getCurrentLanguage() == "cn" {
            lan = "zh-cn"
        } else if getCurrentLanguage() == "zh-tw" {
            lan = "zh-tw"
        }
        #if DEBUG
           env = "dev"
